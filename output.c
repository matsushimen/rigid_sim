...
void molsposition(void){
	...
	for(step = step; step < STEP; step++){
        ...
        if(save == 2)
        {
            FILE *pp;
            char matrixfile[128];
            sprintf(matrixfile, "result/breakcountmatrix%lld.txt",step);
		...
		else
		{
            ...
            for(i = 0; i < Number; i++)//力の計算
			{
                ...
                /////////////////////////ボックスミュラー法によるノイズの生成/////////////////////////////
                p1 = sqrt(2 * 3 * NENSEI * k_B *T*mlcl[i].r)*sqrt(-2.0*log(genrand_real3()));
				p2 = sqrt(2 * 3 * NENSEI * k_B *T*mlcl[i].r)*sqrt(-2.0*log(genrand_real3()));
                ...
                ////////硬いところを作る////////
                if((mlcl[i].scale == 0.0)&&(mlcl[mlcl[i].breakpair2_a].scale==0.0))
                {
                    mlcl[i].f[X] += LS2x(i,mlcl[i].breakpair2_a,RIGID);
                    mlcl[i].f[Y] += LS2y(i,mlcl[i].breakpair2_a,RIGID);
                    mlcl[i].f[Z] += LS2z(i,mlcl[i].breakpair2_a,RIGID);
                }
                if((mlcl[i].scale == 0.0)&&(mlcl[mlcl[i].breakpair2_b].scale==0.0))
                {
                    mlcl[i].f[X] += LS2x(i,mlcl[i].breakpair2_b,RIGID);
                    mlcl[i].f[Y] += LS2y(i,mlcl[i].breakpair2_b,RIGID);
                    mlcl[i].f[Z] += LS2z(i,mlcl[i].breakpair2_b,RIGID);
                }
                    ...
                    for(j = 1; j < list_no[i] + 1; j++)
                    {
                        ...
                        if((mlcl[mlcl[i].bond_pair].bond_pair != i)&&(mlcl[i].bond_pair != -1))//片思いなら外す
                        {
                            mlcl[i].bond_pair = -1;
                                ...
                                &&(mlcl[i].scale != 0.0)&&(mlcl[m].scale != 0.0))//硬い部分でない
                        {
                            ////////くっつく先のペアを互いに記憶///////
                            mlcl[i].bond_pair = m;
                            mlcl[m].bond_pair = i;
            ...
            if((step%10000000 == 0)&&(step!=0))
            {
                ...
                char filename[128];
                sprintf(filename, "result/breakcountmatrix%lld.txt",step);
                ...
                sprintf(logfilename, "result/position%lld.txt",step);
    ...
    //テキストファイルへの最終書き出し
    FILE *fp;
    fp = fopen("result/breakcountmatrix.txt","wt");
